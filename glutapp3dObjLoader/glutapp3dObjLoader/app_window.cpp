...
void AppWindow::loadModel(int model)
{
	...
	std::cout << "Loading space " << file7 << "...\n";
	if (!_gsm7.load(file7))
		std::cout << "Error! you fucked up\n";
	printInfo(_gsm7);
	_gsm7.scale(f7); // to fit our camera space
	man.build(_gsm7);
	redraw();
...
void AppWindow::glutKeyboard(unsigned char key, int x, int y)
{
	...
	case 'm': orbit(); joe = false; redraw(); break;
	case 'n': ranorbit(); joe = false; redraw(); break;
	case 'y': mranorbit(); redraw(); break;
...
void AppWindow::glutDisplay()
{
	...
	GsMat rx, ry, stransf, spacex, spacey, spacexxx, spaceyyy, oogabooga,jetx,jety, man1;
	rx.rotx(_rotx);
	ry.roty(_roty);
	spacex.translation(spacexx, spaceyy, spaceships);
	jetx.translation(jx, jy, jz);
	jety.translation(jx, jy, jz);
	man1.translation(m1, m2, m3);
	stransf = rx*ry;
	...
	man.draw(man1/*spacex*spacey*/, sproj, _light);
...
void AppWindow::move1() {
	...
	for (int i = 0; i < cntrlpoint.size(); i++) {
		...
		cntrlpoint[i] = GsVec((i), (i - 1)*(i - 2), -((i - 2)*(i - 2)*(i - 2) / 1));
